...
export default function ItemsList() {
  ...
  const projects = data.projects.edges.map(item => (
    ...
    >
      <button
        className="popupWindowLinkButton project-title-button"
        onClick={() => createWinboxInstance(item)}
      >
        {item.node.frontmatter.listName}
      </button>
      <div className="project-details">
        <span className="project-meta">
          {`lrwxr-xr-x 1 samarth admin ${item.node.frontmatter.added} ${item.node.frontmatter.slug} ->`}
        </span>
        <a className="project-link" href={item.node.frontmatter.popupGithubLink}>
